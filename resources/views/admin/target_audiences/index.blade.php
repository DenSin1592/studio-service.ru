...
@section('title') {{ 'Каталог целевых аудиторий' }} @stop
...
@section('content')
    <div class="node-list element-list-wrapper" data-sortable-wrapper="">
        ...
        <div data-sortable-container="">
            @include('admin.target_audiences._list', ['lvl' => 0])
            @include('admin.shared._pagination_links', ['paginator' => $modelList])
        </div>

        @include('admin.shared.resource_list.sorting._commit', [
             'updateUrl' => route(\App\Http\Controllers\Admin\EssenceControllers\TargetAudiencesController::ROUTE_UPDATE_POSITIONS),
             'reloadUrl' => route(\App\Http\Controllers\Admin\EssenceControllers\TargetAudiencesController::ROUTE_INDEX)
             ])

        <div>
            <a href="{{ route(\App\Http\Controllers\Admin\EssenceControllers\TargetAudiencesController::ROUTE_CREATE) }}" class="btn btn-success btn-xs">Добавить ЦА</a>
        </div>
    </div>
@stop
