    ...
    @foreach ($models as $model)
        <li data-element-id="{{ $model->id }}">
            <div class="element-container">
                ...
                <div class="name">
                    <a href="{{ route(\App\Http\Controllers\Admin\CompetenciesController::ROUTE_EDIT, [$model->id]) }}"
                       style="margin-left: {{ 0 * 0.5 }}em;">{{ $model->name }}</a>
                </div>

                @include('admin.shared._list_flag', ['element' => $model, 'action' => route(\App\Http\Controllers\Admin\CompetenciesController::ROUTE_TOGGLE_ATTRIBUTE, [$model->id, 'publish']), 'attribute' => 'publish'])
